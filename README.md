...
בתוך ספריית Apple Music שלך. הדף קורא את הפלייליסט הזה (דרך MusicKit JS) בלחיצה על
"בדוק שירים חדשים", ובודק כל שיר חדש מול **כל** הפלייליסטים וה-Liked Songs הקיימים שלך
ב-Spotify (לא רק מול פלייליסט היעד) - כדי לא ליצור כפילויות. התוצאה מוצגת כרשימה עם
תיבות סימון, ושום דבר לא נכתב לחשבון שלך עד שתלחץ/י "אשר והוסף". הכלי אף פעם לא מוחק
או מסיר שירים - רק מוסיף.

בלחיצה על האישור:
1. השירים המסומנים נוספים לפלייליסט ב-Spotify (בשם הנתון, למשל "Shazam").
2. כל השירים החדשים (גם אלה שכבר קיימים אצלך ב-Spotify) נוספים לספריית Apple Music
   שלך (My Music) - כי Shazam רק מכניס אותם לפלייליסט, לא לספרייה עצמה.

מצב הסנכרון (אילו שירים כבר נבדקו) נשמר ב-localStorage של הדפדפן, כך שכל בדיקה
מציגה רק שירים חדשים.
...
4. לחץ "בדוק שירים חדשים" - הכלי יציג רשימה של שירים חדשים מ-Shazam שלא נמצאים
   באף פלייליסט או ב-Liked Songs שלך ב-Spotify.
5. בטל סימון לשירים שלא רוצים להוסיף (אופציונלי), ולחץ "אשר והוסף ל-Spotify".

הרץ שוב בכל פעם שתרצה לבדוק שירים חדשים - הכלי מדלג אוטומטית על שירים שכבר נבדקו.
ה-developer token של Apple בתוקף ל-6 חודשים וה-Spotify token מתחדש אוטומטית, אז
בדרך כלל לא תצטרך להתחבר מחדש בכל פעם.

## הגדרות אופציונליות

- שם פלייליסט Shazam ב-Apple Music - אם שונה מ-"Shazam Library" אצלך
- שם פלייליסט היעד ב-Spotify - ברירת מחדל "Shazam"

## מגבלות ידועות

- הכלי אף פעם לא מוחק או מסיר שירים מפלייליסטים או מהספרייה - רק מוסיף, ורק אחרי אישור.
- בדיקת הכפילויות קוראת את כל הפלייליסטים וה-Liked Songs שלך ב-Spotify, אז ככל
  שיש לך יותר פלייליסטים/שירים, "בדוק שירים חדשים" ייקח יותר זמן.
